<x-filament-panels::page>
    <x-filament::section
        heading="System Backups"
        description="Generate and manage database backup files stored in private application storage."
        icon="heroicon-o-circle-stack"
    >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="max-w-2xl text-sm text-gray-600 dark:text-gray-300">
                Backups may contain sensitive tenant, billing, and token-related data. Store downloaded files securely and delete old copies when they are no longer needed.
            </div>
        ...
        @if (empty($backupFiles))
            <div class="mt-6 rounded-lg border border-dashed border-gray-300 p-8 text-center dark:border-gray-700">
                ...
                <h3 class="mt-3 text-base font-semibold text-gray-950 dark:text-white">No backups available</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Create the first backup when you are ready to export a private copy.</p>
            </div>
        @else
                ...
                <div class="divide-y divide-gray-200 dark:divide-gray-800">
                    @foreach ($backupFiles as $file)
                        <div class="grid gap-3 px-4 py-4 md:grid-cols-[1fr_2fr_1fr_1fr] md:items-center">
                            <div>
                                <div class="text-xs font-medium uppercase text-gray-500 md:hidden">Created</div>
                                <div class="font-mono text-sm text-gray-700 dark:text-gray-300">{{ $file['created_at'] }}</div>
                            </div>

                            <div class="min-w-0">
                                <div class="text-xs font-medium uppercase text-gray-500 md:hidden">File</div>
                                <div class="truncate font-mono text-sm font-semibold text-gray-950 dark:text-white" title="{{ $file['name'] }}">{{ $file['name'] }}</div>
                            </div>

                            <div>
                                <div class="text-xs font-medium uppercase text-gray-500 md:hidden">Size</div>
